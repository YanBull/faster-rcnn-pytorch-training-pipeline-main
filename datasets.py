...
def extract_info_from_xml(xml_file, needed_labels:set):
    ...
    for elem in root:
        ...
        # Get details of the bounding box 
            ...
            if len(needed_labels) == 0 or (len(needed_labels) > 0 and bbox["class"] in needed_labels):
...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.save_aug_preview = save_aug_preview        
    ...
    def __getitem__(self, idx):        
        ...
        if self.use_train_aug: # Use train augmentation if argument is passed.
            ...
            if self.save_aug_preview is not None and os.path.exists(self.save_aug_preview) and image_id.item()%20==0:
                # print("Saving augmentation preview")
                aug_preview_image = image_resized.detach().clone().permute(1, 2, 0).numpy()
                aug_preview_image = np.array(aug_preview_image*255, dtype=np.uint8)
                cv2.imwrite(os.path.join(self.save_aug_preview, "train_augmentations", "aug-preview-" + str(image_id.item()) + ".jpg"), aug_preview_image)
...
def create_train_dataset(
    ...
    train_dataset = CustomDataset(
        ...
        save_aug_preview=save_aug_preview
    )
