...
def parse_opt():
    ...
    parser.add_argument(
        '-sap','--save-aug-preview', 
        action='store_true',
        dest='save_aug_preview', 
        help='whether to save generated augmentations'
    )
    parser.add_argument(
        '-ca', '--cosine-annealing', 
...
def main(args):
    ...
    if args['img_size'] == 1:
        print("Choosing optimal image size to feed into network")    
        _ , IMAGE_SIZE = data.tools.find_smallest_image(TRAIN_DIR_IMAGES)
    else:
        IMAGE_SIZE = args['img_size']

    print("Image size: ", IMAGE_SIZE)
    
    if args["save_aug_preview"] and not os.path.exists(os.path.join(OUT_DIR, "train_augmentations")):
        os.mkdir(os.path.join(OUT_DIR, "train_augmentations", ""))
